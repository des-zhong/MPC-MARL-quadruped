"""Evaluate AS2 walking, dribbling, and shooting policies in one rollout."""
...
DEFAULT_LOCAL_POLICY_DIRS = {
    "walk": ROOT_DIR / "wandb/as2_walk-3a6g1def/files/tmp/legged_data",
    "dribble": ROOT_DIR / "wandb/as2_dribble-cp9m21ay/files/tmp/legged_data",
    "shoot": ROOT_DIR / "wandb/as2_shoot-bve3isir/files/tmp/legged_data",
}
...
def resolve_wandb_policy_files(
    wandb_run,
    checkpoint,
    *,
    skill=None,
    ...
    """Resolve a policy without allowing another W&B run to overwrite it.

    The historical four-value return remains the default for existing callers.
    Low-level policy loaders request the optional fifth metadata value.  Set
    ``refresh=True`` to replace a pinned cache entry for a mutable checkpoint;
    ``None`` honors ``DRIBBLEBOT_WANDB_REFRESH``.
    """
    ...
    if skill is not None:
        remote_skill = "walk" if skill in ("walk", "approach") else skill
        remote_dirs.append(f"tmp/legged_data/{remote_skill}")
    remote_dirs.append("tmp/legged_data")

    body_path = restore_wandb_file(
        run_path,
        [f"{directory}/body_{suffix}.jit" for directory in remote_dirs],
        cache_root=cache_root,
        refresh=refresh,
    )
    adaptation_module_path = restore_wandb_file(
        run_path,
        [
            f"{directory}/adaptation_module_{suffix}.jit"
        ...
        ac_weights_path = restore_wandb_file(
            ...
            (
                [
                    ...
                    for directory in remote_dirs
                    for candidate in (
                        f"{directory}/ac_weights_{suffix}.pt",
                        f"{directory}/ac_weights_last.pt",
                    )
                ]
                if suffix == "latest"
                else [
    ...
    try:
        config_path = restore_wandb_file(
            run_path,
            ["config.yaml"],
            cache_root=cache_root,
            refresh=refresh,
        ...
        config_path = find_local_wandb_config(run_path) if local_config_fallback else None

    result = (body_path, adaptation_module_path, ac_weights_path, run_path)
    ...
    *,
    ...
    if policy_metadata is None:
        ...
        if action_clip is not None:
            policy_metadata["action_clip"] = float(action_clip)
            policy_metadata["action_clip_source"] = "explicit"
    ...
    policy_metadata["expected_history_dim"] = expected_history_dim
    policy_metadata["policy_type"] = policy_type
    print(f"  expected obs_history dim: {expected_history_dim}")
    print(
        f"  action clip: {policy_metadata['action_clip']} "
        f"({policy_metadata['action_clip_source']})"
    )
    body_checksum = policy_metadata["artifacts"]["body"]["sha256"]
    adaptation_checksum = policy_metadata["artifacts"]["adaptation_module"]["sha256"]
    print(f"  body sha256: {body_checksum}")
    print(f"  adaptation module sha256: {adaptation_checksum}")
    ...
    return {
        ...
        "source": source,
        "action_clip": float(policy_metadata["action_clip"]),
...
def load_local_phase_policy(args, phase, checkpoint):
    ...
    run_dir = (
        getattr(args, f"{phase}_run_dir")
        or getattr(args, f"{skill}_policy_dir")
        or DEFAULT_LOCAL_POLICY_DIRS[skill]
    )
    ...
    config_path = find_policy_config_path(body_path, (run_dir,))
...
def load_wandb_phase_policy(args, phase, checkpoint):
    ...
    body_path, adaptation_module_path, ac_weights_path, run_path, policy_metadata = resolve_wandb_policy_files(
        wandb_run,
        checkpoint,
        skill=PHASE_TO_SKILL[phase],
...
def load_phase_policies(args):
    ...
    configured_policy_dirs = any(
        getattr(args, f"{skill}_policy_dir")
    ...
    # The new source contract matches test.bash and the high-level scripts.
    # With no source arguments at all, use the three run-specific local AS2
    # folders instead of accidentally reusing one policy for every phase.
    use_default_local_sources = (
        source_kind == "local"
        or (
            source_kind is None
            and configured_policy_dirs
        )
        or (
            source_kind is None
            and not using_phase_sources
            and not args.wandb_run
            and not args.local
            and args.run_dir == DEFAULT_RUN_DIR
            and args.body is None
            and args.adaptation_module is None
            and args.pt is None
        )
    )
    if use_default_local_sources:
        for phase in PHASES:
    ...
    if args.wandb_run and not args.local:
        ...
        config_path = find_policy_config_path(body_path, (args.run_dir,))
        record = load_policy_record(
            "all phases",
            body_path,
            adaptation_module_path,
...
def clip_policy_action(action, policy_record):
    """Apply the actuator-action range used by this policy during training."""

    clip = float(policy_record["action_clip"])
    return torch.nan_to_num(action, nan=0.0, posinf=clip, neginf=-clip).clamp(-clip, clip)
...
def run(args):
    ...
    elif metadata_config:
        config_path = Path(metadata_config)
    else:
        config_anchor = dribble_record["body_path"].parent
        config_path = find_config_path(args, config_anchor)
    print(f"Using config: {config_path or 'built-in ball rollout config'}")

    raw_action_clip = max(float(record["action_clip"]) for record in phase_policies.values())
    print(f"Using raw environment action clip: {raw_action_clip}")
    if args.validate_policies_only:
        print("Validated walk, dribble, and shoot policies; rollout was not started.")
        return

    env = make_env(args, config_path, raw_action_clip=raw_action_clip)
    ...
    try:
        for step in trange(args.steps, desc="Walk-dribble-shoot"):
            ...
            with torch.no_grad():
                ...
                action = clip_policy_action(action, phase_policy)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load all three phase policies from local --*-policy-dir folders or online W&B. "
            "With no source arguments, the run-specific local AS2 folders are used."
        ),
    )
    parser.add_argument(
        "--walk-policy-dir",
        default=None,
        help="Local walking checkpoint directory used by the approach phase.",
    )
    parser.add_argument("--dribble-policy-dir", default=None, help="Local dribbling checkpoint directory.")
    parser.add_argument("--shoot-policy-dir", default=None, help="Local shooting checkpoint directory.")
    ...
    parser.add_argument(
        "--validate-policies-only",
        action="store_true",
        help="Load and validate all three policies, then exit before creating the simulator.",
    )
