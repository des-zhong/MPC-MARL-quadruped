...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Train Go1 dribbling with normalized command-scale rewards.")
...
def train_robot(args=None, headless=True):
    ...
    if args.resume and not args.resume_run:
        raise ValueError("--resume requires --resume-run for a compatible dribbling checkpoint.")
    ...
    wandb.init(
      ...
      project=args.project or "dribbling",
