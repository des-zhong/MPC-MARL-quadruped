...
from isaacgymenvs.tasks.go1_kick import Go1Kicker
...
class RewardTerms:
    def __init__(self, env: Go1Kicker):
        self.env = env
    ...
    def _reward_dribbling_robot_ball_vel(self):
        ...
        condition = (self._reward_dribbling_ball_vel() < 0.5).float()
        return rew_dribbling_robot_ball_vel * condition + (1 - condition)
    ...
    def _reward_dribbling_robot_ball_pos(self):
        ...
        condition = (self._reward_dribbling_ball_vel() < 0.5).float()
        return rew_dribbling_robot_ball_pos * condition + (1 - condition)
    ...
    def _reward_dribbling_ball_vel(self):
        ...
        angle_diff_in_pi = torch.abs(wrap_to_pi(angle_diff))
        
        ball_vel_norm = torch.norm(self.env.object_lin_vel[:, :2], dim=-1)
        tolerance_angle = self.env.cfg["env"]["rewards"]["rewardParams"]["angle_tolerance_half_range"]
        angle_condition = (angle_diff_in_pi < (np.pi / 180 * tolerance_angle)).float()

        return torch.clip(ball_vel_norm, max=1.0) * angle_condition
    ...
    def _reward_dribbling_robot_ball_yaw(self):
        ...
        unit_command_vel = self.env.commands[:, :2] / torch.clip(torch.norm(
            self.env.commands[:, :2], dim=-1
        ), min=1e-6).unsqueeze(dim=-1)
    ...
    def _reward_tracking_lin_vel_PID(self):
        ...
        v_dog_target = torch.zeros(self.env.num_envs, 2, device=self.env.device)
        lin_vel_error = torch.sum(
            torch.square(v_dog_target[:, :2] - self.env.base_lin_vel[:, :2]),
            dim=1,
        )
